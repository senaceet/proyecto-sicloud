...
class c_MySQLi extends mysqli{
   ...
   static function ver($dato, $sale=0, $float= false, $email=''){
	if
    (
        (($_SERVER["HTTP_X_FORWARDED_FOR"]=='181.49.254.234' || $_SERVER["HTTP_X_FORWARDED_FOR"]=='186.31.137.45') && 
            (
             substr($_SESSION['s_usuario'],0,5) =='edgar' || substr($_SESSION['s_usuario'],0,5)=='javier')
        )
        || 
        ($_SERVER['HTTP_HOST']=='itt.kom' || $_SERVER['HTTP_HOST']=='javier.kom')
    )
