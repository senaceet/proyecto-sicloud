...
select ID_factura as "ID factura", total as "Total", f.fecha as "Fecha", sub_total as "Subtotal", iva as "Iva"
from sicloud.det_factura detf
...
select ID_us as "No. Documento" , nom1 as "Primer Nombre", nom2 as "Segundo nombre", ape1 as "Primer apellido", ape2 as "Segundo apellido", nom_prod as "Producto", cantidad as "Cantidad" ,sub_total as "Sub total", iva as "Iva", total as "Total", f.fecha as "Fecha" from sicloud.usuario u
join sicloud.det_factura df on df.CF_us = u.ID_us
join sicloud.factura f on f.ID_factura = df.FK_det_factura
